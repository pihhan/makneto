...
na rozdíl od SIP\ref{sip} má XMPP jasnì definovaný formát posílání zprávy
...
napøíklad statistické údaje vyu¾ití slu¾by.
...
která popisuje vlastnosti dat plynoucí vysílaných nebo pøijímaných daným 
elementem. Formát vlastních dat popisuje struktura \verb|GstCaps|, která
...
Takovým elementem je \verb|GstBin|, který je implementací bì¾ného elementu.
Speciálním pøípadem binu je GstPipeline, která implementuje navíc jenom 
nastavení zdroje èasu a prodlevu. 
...
nastavuje stav pouze na objekt pipeline, která postupnì mìní stavy obsa¾ených
elementù. Po pøejití v¹ech elementù na po¾adovaný stav teprve nastaví nový
stav na vlastní objekt. Stavy se mìní postupnì po jednom asynchronnì, pouze
zastavení pipeline se provádí synchronnì.
...
do stavu PAUSED bez úvodního pøednaètení, aby pøi pøijetí dat ji¾ byly schopny
...
sledujícího sbìrnici zpráv. Zprávy chodí asynchronnì, ka¾dá zpráva má svùj typ
a pøípadnou datovou strukturu. Zprávy jsou zasílány napøíklad pøi zmìnì stavu
...
negativní vliv na kvalitu pøehrávání.
...
jako \verb|GstBin| pøidaný do øetìzce a komunikace knihovny k programu
...
\begin{figure}[ht]
\begin{center}
  \includegraphics[width=10cm,keepaspectratio]{farsight}
...
média inicializuje seznam kodekù dostupných v systému. Pro ka¾dého pøíjemce
...
výmìna není souèástí knihovny farsight.
